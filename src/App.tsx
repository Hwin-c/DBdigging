...
const GalaxyButton = ({ title, subtitle, onClick }: { title: string, subtitle: string, onClick: () => void }) => {
  ...
  return (
    <motion.button
      onClick={handleClick}
      className="relative w-[340px] h-[340px] rounded-full flex items-center justify-center group outline-none"
      initial={{ scale: 1, opacity: 1 }}
      animate={isZooming ? { scale: 15, opacity: 0 } : { scale: 1, opacity: 1 }}
      transition={{ duration: 0.6, ease: "easeInOut" }}
      whileHover={{ scale: 1.05 }}
    >
      {/* Galaxy background rings */}
      <div className="absolute inset-0 rounded-full border border-white/20 animate-spin-slow" style={{ borderStyle: 'dashed' }}></div>
      <div className="absolute inset-4 rounded-full border border-white/30 animate-spin-slow-reverse" style={{ borderStyle: 'dotted' }}></div>
      <div className="absolute inset-8 rounded-full border border-white/10 animate-spin-slow"></div>
      <div className="absolute inset-12 rounded-full border border-white/5 animate-spin-slow-reverse" style={{ borderStyle: 'dashed' }}></div>
      
      {/* Central glow */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(255,255,255,0.4)_0%,_transparent_60%)] rounded-full mix-blend-screen opacity-60 group-hover:opacity-90 transition-opacity duration-500"></div>
      
      {/* Fake stars */}
      <div className="absolute top-[20%] left-[20%] w-1 h-1 bg-white rounded-full shadow-[0_0_5px_white]"></div>
      <div className="absolute top-[80%] right-[20%] w-1.5 h-1.5 bg-white rounded-full shadow-[0_0_8px_white]"></div>
      <div className="absolute bottom-[30%] left-[10%] w-1 h-1 bg-white/80 rounded-full"></div>
      <div className="absolute top-[50%] right-[10%] w-1 h-1 bg-white/90 rounded-full"></div>
      <div className="absolute top-[10%] left-[60%] w-2 h-2 bg-white rounded-full shadow-[0_0_10px_white]"></div>

      <div className="relative z-10 text-center flex flex-col gap-2 pointer-events-none">
        <span className="text-3xl font-sans tracking-widest text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] font-medium">{title}</span>
        <span className="text-lg font-sans text-white/90">{subtitle}</span>
      </div>
    </motion.button>
  );
};
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white overflow-hidden font-sans selection:bg-white/30">
      ...
      <header className="relative z-20 flex items-center justify-between px-8 py-6 border-b border-white/10 bg-black/20 backdrop-blur-md">
        <div className="flex items-center gap-8 w-1/3">
          ...
          {currentView === 'map' && (
            <nav className="flex items-center gap-4 bg-white/5 rounded-full p-1 border border-white/10">
              <button
                onClick={() => { setCurrentTab('genre'); setCurrentNode(null); }}
              ...
              <button
                onClick={() => { setCurrentTab('artist'); setCurrentNode(null); }}
                className={`px-6 py-2 rounded-full text-sm font-medium tracking-wider uppercase transition-all ${
        ...
        <div className="flex justify-center w-1/3 z-50">
          <SearchBar onNodeSelect={handleSearchSelect} />
        </div>

        <div className="flex items-center justify-end gap-6 w-1/3 z-40">
          {/* Breadcrumb / Current Path */}
          {currentView === 'map' && (
            <div className="hidden md:flex items-center gap-2 text-sm text-white/50 font-mono">
              <button onClick={() => { setCurrentTab(currentTab); setCurrentNode(null); }} className="hover:text-white transition-colors">ROOT</button>
              {currentNode && (
                <>
                  <span>/</span>
          ...
          <div className="flex items-center gap-4 text-sm font-mono border-l border-white/10 pl-6">
            {user ? (
              ...
              <button onClick={login} className="bg-transparent text-white px-4 py-2 text-sm font-bold hover:text-white/70 transition-colors text-center leading-[1.2]">
                Login
              </button>
            )}
          </div>
      ...
      {/* Main Content Area */}
      <main className="relative w-full h-[calc(100vh-85px)] z-10 flex">
        {currentView === 'home' ? (
          <div className="w-full h-full flex items-center justify-center gap-32 z-10 mb-16">
            <GalaxyButton 
              title="TRACKS"
              subtitle="(곡 중심)"
              onClick={() => { setCurrentTab('artist'); setCurrentView('map'); }}
            />
            <GalaxyButton 
              title="GENRES"
              subtitle="(장르 중심)"
              onClick={() => { setCurrentTab('genre'); setCurrentView('map'); }}
            />
          </div>
        ) : (
